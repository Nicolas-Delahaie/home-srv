...
- If the product name is already a single, reasonably short word, keep it as-is: `frigate`, `hermes`, `mosquitto`, `traefik`, `authelia`, `glances`, `portainer`, `immich`.
...
- If a natural acronym doesn't exist or would be ambiguous, concatenate the full words instead, with no separator: `openwu` (Open WebUI), `gharunner` (GitHub Actions runner), `immichdb`, `immichml`, `immichredis`. Length matters less here than for the two rules above, especially for a service that isn't exposed through Traefik.
