...
- **DNS + DHCP**: AdGuard Home (filtering, local domain resolution, DHCP)
...
- Traefik and AdGuard use `envsubst` at runtime (`traefik_static.yml` / `adguard/adguardhome.yaml` → final config)
